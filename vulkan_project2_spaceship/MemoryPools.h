...
public:
	...
	Vertex* GetData()
	{
		if (m_data != nullptr)
			delete[] m_data;
		m_data = new Vertex[static_cast<size_t>(m_count)];
		for (int i = 0; i < m_count; i++)
		{
			const Vertex* ref = this->Refer(i);
			if (ref != nullptr)
				m_data[i] = *ref;
			else
				m_data[i] = Vertex();
...
public:
	...
	uint16_t* Refer(const int index) {
		Node* _iter = m_pStart;
		for (int _iter_count = 0; _iter != nullptr && _iter_count < index / MAX_VALUE_PER_POOLS; _iter = _iter->GetNext(), _iter_count++);
		if (_iter != nullptr) return &_iter->m_points[index % MAX_VALUE_PER_POOLS];
		else return NULL;
