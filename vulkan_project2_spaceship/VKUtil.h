...
void endSingleTimeCommands(VkCommandPool& commandPool, VkCommandBuffer commandBuffer, VkQueue targetQ)
{
    ...
    vkQueueSubmit(targetQ, 1, &submitInfo, VK_NULL_HANDLE);
    vkQueueWaitIdle(targetQ);

    vkFreeCommandBuffers(g_device, commandPool, 1, &commandBuffer);

}
void createBuffer(VkPhysicalDevice& _physicalDevice, VkDevice& _device, VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
void generateMipmaps(VkCommandPool& commandPool, VkQueue& targetQ, VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
    ...
    endSingleTimeCommands(commandPool, commandBuffer, targetQ);
}
...
void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
    ...
    allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = memRequirements.size;
    allocInfo.memoryTypeIndex = findMemoryType(g_physicalDevice, memRequirements.memoryTypeBits, properties);
...
void copyBuffer(VkBuffer src, VkBuffer dst, VkDeviceSize size, VkCommandPool& targetCommandPool, VkQueue targetQ) {

    VkCommandBuffer commandBuffer = beginSingleTimeCommands(targetCommandPool);
    VkBufferCopy copyRegion{};
    copyRegion.size = size;
    vkCmdCopyBuffer(commandBuffer, src, dst, 1, &copyRegion);
    endSingleTimeCommands(targetCommandPool, commandBuffer, targetQ);
}
void copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, VkCommandPool& targetCommandPool, VkQueue targetQ) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(targetCommandPool);
...
void transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels,VkCommandPool& targetCommandPool, VkQueue targetQ) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(targetCommandPool);
    VkImageMemoryBarrier barrier{};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
